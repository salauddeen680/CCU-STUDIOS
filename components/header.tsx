...
export function Header() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // WebView popup block hone par direct fallback
      if (error?.code === "auth/popup-blocked" || error?.code === "auth/operation-not-supported-in-this-environment") {
        ...
        alert(error?.message || "Login unavailable right now.")
      }
    } finally {
  ...
  return (
    <>
      {/* 🔥 100% PURE GLASSMORPHIC HEADER: Seamless transparency blending into the red glow */}
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/[0.08] bg-black/15 backdrop-blur-xl transition-all shadow-[0_4px_30px_rgba(0,0,0,0.15)]">
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div
              ...
              className="overflow-hidden border-t border-white/10 bg-black/40 backdrop-blur-2xl"
            >
        ...
        {/* 📱 MOBILE NAV MENU: Seamless semi-transparent backdrop */}
        <AnimatePresence>
          {open && (
            <motion.nav
              initial={{ height: 0, opacity: 0 }}
              animate={{ height: "auto", opacity: 1 }}
              exit={{ height: 0, opacity: 0 }}
              className="overflow-hidden border-t border-white/10 bg-black/70 backdrop-blur-2xl md:hidden"
            >
